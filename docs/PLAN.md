...
- [ ] `⌘⇧[` / `⌘⇧]` 環狀切換分頁；游標在編輯器裡按也有效（不被 CM6 的 `⌘[` 縮排吃掉）；`⌘W` 關分頁，未存內容有確認
- [ ] `⌘/` 浮層：Tab 只在面板內環繞不跑到工具列；Esc 關閉後直接打字，焦點已回編輯器
- [ ] 設定面板開啟後 Tab 同樣關得住
- [ ] 視窗縮到最矮再開 `⌘/`：面板可捲動，方向鍵也能捲
- [ ] 切換介面語言後 `<html lang>` 跟著變（開發者工具查屬性值，或聽 VoiceOver 換語音）
- [ ] 系統偏好開「減少動態」→ 浮層開關無過場，且**仍然關得掉**（關閉邏輯讀 `transitionDuration` 分流，0 會走立即完成分支）
- [ ] 語言下拉出現四個選項（正體中文／English／简体中文／日本語）；切到简体中文與日本語後，原生選單列也跟著換（選單是 JS 重建，與 DOM 翻譯是兩條路）
- [ ] 切到日本語後狀態列不破版——日文字串比中文長（「保存済み」vs「已儲存」），儀表寬度固定
- [ ] 舊使用者升級（`locales/` 已存在但無 `zh_Hans.json`／`ja.json`）→ 新語言仍被種下，下拉看得到（種子是逐檔 `exists()` 判斷，不是整個目錄）
