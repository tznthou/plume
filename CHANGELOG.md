...
- 分頁快捷鍵：`⌘⇧[` / `⌘⇧]` 環狀切換分頁、`⌘W` 關閉當前分頁，選單列新增「分頁」選單。先前分頁只能用滑鼠切，或先把焦點移到分頁列——寫作時焦點在編輯器裡，等於沒有鍵盤入口
- 支援系統「減少動態」偏好：開啟後介面過場一律歸零（WCAG 2.3.3）

### 修復

- 快捷鍵浮層與設定面板都宣告了 `aria-modal` 卻沒有焦點管理：焦點不會進入面板、Tab 會走到背後的工具列、關閉後焦點留在遊蕩處。宣告 modal 卻不關住焦點比不宣告更糟——螢幕閱讀器照著把背景藏起來，鍵盤卻還站在那裡。現在焦點關得住（Tab 在面板內環繞），關閉後回到原本的位置
- 切換介面語言時 `<html lang>` 沒有跟著更新，螢幕閱讀器會用中文語音念英文介面
- 快捷鍵浮層在矮視窗下超出畫面且無法捲動，上下都被裁掉
- 正體中文介面的快捷鍵浮層標題未翻譯（v0.12 起）
- 狀態列儀表的無障礙標籤掛在無語意元素上，被輔助技術忽略

## [0.14.1] - 2026-07-26

### 安全

- 自訂主題 CSS 淨化改為「偵測 + 整份拒絕」：原「就地清除」實作存在可繞過的外連路徑，且改寫本身可能破壞合法 CSS。新架構判斷失準只會偏向拒絕載入（fail-close），Rust 端以 19 個攻擊 payload 迴歸測試鎖定

### 修復

- 硯台主題下匯出下拉選單被分頁列蓋住、Export PDF 無法點擊（stacking context 層級錯誤，v0.11 起存在）

### 變更

- i18n 翻譯來源收斂為單一份 `locales/*.json`（前後端編譯期內嵌），修正 v0.13 起 25 個鍵只存在 TS 側的漂移，語言包種子檔自此完整
- 「開啟語言包／佈景主題資料夾」改用 opener plugin 官方 API，取代手寫三平台命令
- CI 發版流程補跑 Rust 測試（先前僅跑前端 Vitest）

## [0.14.0] - 2026-07-24

### 新增

- 分頁獨立歷史：每個分頁保有獨立的 undo/redo、游標位置與選取範圍，切分頁再切回不會丟失編輯記錄
- 右鍵選單：編輯區右鍵提供剪下/複製/貼上/全選，預覽區提供複製/全選。取代先前的「停用全域右鍵選單」，同時阻擋瀏覽器「重新載入」避免內容遺失
- Mermaid 圖表 PDF 匯出：PDF 匯出時懶載入 mermaid 渲染圖表為 SVG，不再匯出原始碼文字
- Office 97 主題選單整合：內建主題選單正式包含 Office 97，含 i18n 翻譯

### 修復

- 分頁切換後預覽/目錄/統計不更新：`EditorView.setState()` 不觸發 CM6 updateListener，補上手動觸發 changeListeners
- 專注模式/打字機模式切分頁一致性：切分頁後 dimming 狀態與選單 checkbox 保持同步

## [0.13.1] - 2026-07-23

### 修復

- Office 97 主題下匯出下拉選單排版錯位
- 目錄按鈕缺少 SVG 圖示
