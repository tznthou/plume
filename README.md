...
開檔即閱讀、落筆即沉浸的桌面 Markdown 工具。讀別人的 `.md` 是渲染好的全幅閱讀，寫自己的有多分頁、專注模式、打字機捲動、KaTeX 數學與 mermaid 圖表。Tauri 2 原生應用——無帳號，放一個 CSS 或 JSON 就能換主題、換語言，不需要外掛市集。
...
| 功能 | 說明 |
|------|------|
| **多分頁編輯** | 同時開多個 `.md`，分頁切換、關閉、未儲存標示（dirty dot）；每個分頁保有獨立的 undo/redo、游標位置與選取範圍——切走再切回不丟編輯記錄。`⌘⇧[` / `⌘⇧]` 環狀切換分頁、`⌘W` 關閉當前分頁，寫作時手不離鍵盤。關閉視窗時逐一確認未儲存分頁 |
| **CodeMirror 6 編輯器** | 行號、Markdown 語法高亮、搜尋取代、undo/redo；注音輸入法組字實測不斷字 |
| **即時預覽** | 「參」分欄下輸入後 50ms 內更新，所見即所得 |
| **右鍵選單** | 編輯區右鍵：剪下/複製/貼上/全選；預覽區右鍵：複製/全選。取代原生右鍵選單，同時擋掉「重新載入」避免內容遺失 |
...
| **匯出 PDF** `⌘P` | File ▸ Export PDF 透過 macOS 原生列印面板輸出向量 PDF——文字可選取、可搜尋、可複製，Mermaid 圖表渲染為 SVG，零外部依賴 |
...
| **本地圖片預覽** | Markdown 中的相對路徑圖片在預覽區即時顯示，透過 Tauri asset protocol 安全載入 |
...
| 功能 | 說明 |
|------|------|
| **佈景主題** | 內建夜航（深色）、硯墨（淺色）、Office 97（懷舊）、Auto（跟隨系統明暗）。Office 97 完整復刻凸起按鈕、點陣 icon 和那個藍底白字的標題列，懷念的味道都是對的 |
| **自訂主題** | 在 `themes/` 資料夾放一個 CSS 就是一套新主題。設定面板即時選擇，內建三套範本（翠綠森林、極光北歐、Office 97）可以直接拿來改 |
| **多語系** | 正體中文、英文、簡體中文、日文四組內建語言包（簡中採在地化用詞非字形轉換；日文尚未經母語者校對）。語言選擇器即時切換——工具列、原生選單、對話框、快捷鍵說明、狀態列全面在地化。在 `locales/` 放一個 JSON 就能加新語言 |
| **設定面板** | 齒輪圖示一按：佈景主題、語言、版本資訊、一鍵檢查更新，全收在一個浮層裡 |
...
flowchart LR
    subgraph Frontend["WebView 前端（Vanilla TS + Vite）"]
        ...
        UX["體驗層<br/>theme（四態主題）<br/>reading-prefs（字型/字級）<br/>focus-mode / typewriter<br/>i18n（多語系）<br/>menu / shortcuts / toc / context-menu"]
        File["file.ts<br/>開檔 / 存檔 / 匯出<br/>多分頁管理 / 文件狀態"]
    ...
    subgraph Rust["Rust 核心（src-tauri）"]
        ...
        Commands["自訂 Commands<br/>grant_scope / get_opened_urls<br/>list_codex_files / pick_codex_root<br/>load_locales / open_locales_dir<br/>delete_codex_folder<br/>load_custom_themes / open_themes_dir<br/>import_theme_file<br/>copy_builtin_theme_template"]
    end
...
**設計原則**：讀寫雙主——開檔進全幅閱讀（閱），動筆切沉浸寫作（撰），讀寫對等而非主從，要邊寫邊對照就用分欄（參）；專注、打字機、即時預覽在寫作態一個不少。多分頁讓你同時開著幾份文件來回切，每個分頁有自己的 undo/redo 和游標，切走再切回不丟東西。Markdown 渲染管線完整留在前端（同步、零 IPC、零 race condition），mermaid 與 KaTeX 是懶載入的後處理；圍繞這條主幹的是體驗層（主題、字型、專注/打字機、多語系、選單、目錄、冊），不改變資料流，只調整呈現。延伸機制走最輕的路——放一個 CSS 檔就是新主題，放一個 JSON 就是新語言，不需要 plugin API 或市集。Rust 端負責檔案 I/O、對話框、系統整合，以及十一個自訂 command——檔案操作兩個（`grant_scope` 外部路徑授權、`get_opened_urls` 冷啟動路徑）、冊三個（`pick_codex_root` 資料夾授權、`list_codex_files` 唯讀列舉、`delete_codex_folder` 白名單移除）、i18n 兩個（`load_locales` 語言包載入、`open_locales_dir` 開啟語言包資料夾）、主題四個（`load_custom_themes` 載入自訂主題含 CSS 淨化、`open_themes_dir` 開啟主題資料夾、`import_theme_file` 匯入主題檔、`copy_builtin_theme_template` 複製內建範本）。
...
│   ├── theme.ts            # 佈景主題（夜航/硯墨/Auto + 自訂主題），matchMedia 監聯系統明暗
│   ├── settings.ts         # 設定面板：主題選擇、更新檢查、版本顯示
│   ├── i18n.ts             # 多語系：語言包載入、即時切換、DOM 翻譯
