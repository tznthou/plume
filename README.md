...
| **匯出 PDF** `⌘P` | File ▸ Export PDF 透過 macOS 原生列印面板輸出向量 PDF——文字可選取、可搜尋、可複製，零外部依賴 |
...
| 功能 | 說明 |
|------|------|
| **佈景主題** | 內建夜航（深色）、硯墨（淺色）、Auto（跟隨系統明暗）。另有三套自訂主題範本：翠綠森林、極光北歐，以及讓人瞬間回到 Windows 95 時代的 Office 97——完整復刻凸起按鈕、點陣 icon 和那個藍底白字的標題列，懷念的味道都是對的 |
| **自訂主題** | 在 `themes/` 資料夾放一個 CSS 就是一套新主題。設定面板（齒輪圖示）即時選擇，內建範本可以直接拿來改 |
